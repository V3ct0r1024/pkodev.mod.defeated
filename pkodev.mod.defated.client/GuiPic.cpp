...
namespace pkodev { namespace gui {
	...
	CGuiPic::CGuiPic() :
		...
		m_path("")
	{
		import::CGuiPic__CGuiPic(m_data, nullptr, 0u);
	}
	...
	std::string CGuiPic::GetIconFile(const std::string& icon)
	{
		...
		bool flag = false;
		const std::size_t pos = icon.rfind('.');

		if (pos != icon.npos) {
			std::string ext = icon.substr(pos + 1);
			std::transform(ext.begin(), ext.end(), ext.begin(), std::tolower);
			for (const std::string& format : { "tga", "png", "bmp" }) {
				if (ext == format) {
					flag = true;
					break;
				}
		...
		if (flag == true) {
			sprintf_s(buf, sizeof(buf), "texture/icon/%s", icon.c_str());
		}
		else {
